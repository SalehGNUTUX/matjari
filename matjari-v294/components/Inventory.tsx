    ...
    {isModalOpen && (
      ...
      <div className="relative">
        ...
        {showCategoryDropdown && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-2xl shadow-2xl z-[500] overflow-hidden max-h-48 overflow-y-auto">
            {categories.filter(cat => cat !== (t.all || 'الكل')).map(cat => (
              <button
                key={cat}
                type="button"
                onPointerDown={e => { e.preventDefault(); setFormData({...formData, category: cat}); setShowCategoryDropdown(false); }}
                className="w-full text-right px-4 py-3 text-sm font-bold dark:text-white hover:bg-primary hover:text-white transition-all border-b border-gray-100 dark:border-gray-700 last:border-0"
              >
                {cat}
              </button>
            ))}
