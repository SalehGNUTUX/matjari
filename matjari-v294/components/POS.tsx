...
export const POS: React.FC<POSProps> = ({
  ...
  const removeOneFromCart = (id: string) => setCart(prev =>
...
<div class="c">
  ...
  <div class="qr-sec">
    ...
    ${qrDataUrl ? `<img src="${qrDataUrl}" width="80" height="80" style="display:block;margin:0 auto" />` : `<div style="width:80px;height:80px;background:#f5f5f5;display:flex;align-items:center;justify-content:center;font-size:9px;color:#aaa;margin:0 auto;border:1px solid #eee">#${pendingSale.id}</div>`}
    <div style="font-size:8px;color:#333;font-family:monospace;margin-top:3px;letter-spacing:1px">${pendingSale.id}</div>
  </div>
  <div class="divider"></div>
          ...
          {products
            ...
            .map(p => (
              <ProductCard key={p.id} product={p} cartQty={cart.find(i => i.id === p.id)?.quantity || 0}
                onAdd={addToCart} onRemoveOne={removeOneFromCart} cur={cur} truncate={truncate} />
            ))}
        </div>
      </div>
        ...
        <div className="lg:hidden space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {products.filter(p=>p.name.toLowerCase().includes(searchQuery.toLowerCase())||p.barcode.includes(searchQuery))
              .map(p=>(<ProductCard key={p.id} product={p} cartQty={cart.find(i=>i.id===p.id)?.quantity||0} onAdd={addToCart} onRemoveOne={removeOneFromCart} cur={cur} truncate={truncate}/>))}
          </div>
          {cart.length>0 && (
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
              <div ref={receiptRef} className="bg-white p-4 mx-auto w-72 font-cairo text-black rounded-xl shadow-sm border border-gray-100" dir="rtl">
                ...
                {pendingSale.paymentMethod === 'debt' && (
                  <div className="mt-2 bg-red-50 border border-red-300 rounded-lg p-2 text-[9px] text-center text-red-700 font-black">
                    ⚠️ هذه الفاتورة دين — المتبقي: {(pendingSale.total - pendingSale.amountPaid).toFixed(2)} {cur}
                  </div>
                )}
                ...
                <div className="mt-3 text-center">
                  <QRCodeSVG id="receipt-qr-svg" value={generateQRData()} size={80} level="H" includeMargin />
                  <p className="text-[8px] text-gray-400 mt-1">{pendingSale.id}</p>
                  <p className="text-[8px] text-gray-400">شكراً لزيارتكم</p>
                </div>
              </div>
            </div>
