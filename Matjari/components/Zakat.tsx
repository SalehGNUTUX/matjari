...
// أسعار الذهب والفضة التقريبية (بالدرهم المغربي)
const APPROX_GOLD_PRICE_PER_GRAM = 900; // درهم/جرام تقريباً
const APPROX_SILVER_PRICE_PER_GRAM = 10;
    ...
    <div className="space-y-5" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {daysToHawl && daysToHawl > 0 && daysToHawl <= 30 && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-xl p-4">
          <p className="text-sm text-blue-700 dark:text-blue-400 font-bold">
            ⏰ {t.hawl_in || 'الحول بعد'} {daysToHawl} {t.days || 'يوماً'} — {new Date(hawlDate!).toLocaleDateString('ar-MA')}
          </p>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-bold text-gray-700 dark:text-gray-300 block mb-1.5">
              🤝 {t.receivables || 'الديون عليك (المبالغ المستحقة لك)'}
            </label>
            <input
              type="number"
              className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
          ...
          <div>
            <label className="text-sm font-bold text-gray-700 dark:text-gray-300 block mb-1.5">
              📉 {t.debts_owed || 'ديونك (ما عليك من ديون)'}
            </label>
            <input
              type="number"
              className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                ...
                <div key={rec.id} className="p-4 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      {new Date(rec.calculatedAt).toLocaleDateString('ar-MA')}
                    </div>
                  </div>
