...
export const Inventory: React.FC<InventoryProps> = ({
  ...
  return (
    ...
    onClick={() => alert(t.scanner_ready || "نظام المسح جاهز، استخدم الماسح الخارجي أو الكاميرا")}
    >
    ...
    {isScannerOpen && (
      <div className="fixed inset-0 bg-black/90 z-[5000] flex flex-col items-center justify-center p-6">
      <div id="inventory-camera-reader" className="w-full max-w-md overflow-hidden rounded-[2.5rem] bg-white border-4 border-primary shadow-2xl"></div>
      <button
      onClick={() => setIsScannerOpen(false)}
      className="mt-8 bg-red-600 text-white px-10 py-4 rounded-2xl font-black active:scale-95 transition-all shadow-lg"
      >
      {t.cancel_scan || 'إلغاء المسح'}
      </button>
      </div>
    )}
