...
export const POS: React.FC<POSProps> = ({
  ...
  const generateQRData = () => {
    ...
    return JSON.stringify({ id: pendingSale.id, store: settings.storeName, total: pendingSale.total, date: new Date(pendingSale.timestamp).toLocaleDateString('ar-MA') });
  };
...
<div class="c">
  ...
  <div class="qr-sec">
    <div style="font-size:8px;color:#555;margin-bottom:4px">مسح للتفاصيل أو المرتجع</div>
    ${qrDataUrl ? `<img src="${qrDataUrl}" width="80" height="80" style="display:block;margin:0 auto;border:1px solid #eee" />` : `<div style="width:80px;height:80px;background:#f5f5f5;display:flex;align-items:center;justify-content:center;font-size:9px;color:#aaa;margin:0 auto;border:1px solid #eee">#${pendingSale.id}</div>`}
    <div style="font-size:8px;color:#888;margin-top:2px">#${pendingSale.id}</div>
  </div>
  <div class="divider"></div>
...
<script>setTimeout(()=>{window.print();setTimeout(()=>window.close(),300)},300);</script>
  ...
  const printReceipt = async (): Promise<boolean> => {
    ...
    const result = await printOrShare(html, pendingSale.id);
    if (!result.success) {
