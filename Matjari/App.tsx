...
const INITIAL_SETTINGS: AppSettings = {
  ...
  security: {
    ...
    requirePasswordForRefund: false,
  },
  lowStockThreshold: 5,
  zakatReminderEnabled: false,
  zakatHawlDate: '',
  zakatMadhab: 'hanafi' as any,
};
...
const App: React.FC = () => {
      ...
      if (sale.customerId && settings.pointsSystemEnabled && sale.pointsInfo) {
        updatedCustomers = customers.map(c =>
          c.id === sale.customerId
            ? { ...c, totalSpent: c.totalSpent + sale.total, points: sale.pointsInfo!.newTotal, lastVisit: Date.now() }
            : c
        );
      }
  ...
  const handleResetSystem = async (): Promise<boolean> => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `matjari-backup-${Date.now()}.json`;
  ...
  if (!currentUser) {
    return <Login users={users} onLogin={handleLogin} initialLanguage={settings.interfaceLanguage} />;
  }
  ...
  return (
    ...
    <Layout activeTab={activeTab} setActiveTab={setActiveTab} settings={settings} currentUser={currentUser} t={t}>
      ...
      {activeTab === 'inventory' && (
        <Inventory products={products} setProducts={setProducts} suppliers={suppliers} settings={settings} currentUser={currentUser} t={t} />
      )}
      {activeTab === 'suppliers' && (
      ...
      {activeTab === 'settings' && (
        <Settings
          ...
          products={products} sales={sales} customers={customers} suppliers={suppliers} t={t}
        />
      )}
    </Layout>
