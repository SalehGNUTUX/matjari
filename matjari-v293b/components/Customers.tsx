...
import { Copy as CopyIcon, Search, UserPlus, Trash2, Edit2, Filter, Award, Phone, Mail, MapPin, Star, X, ChevronLeft, ChevronRight, RotateCcw, Shield, Gift, ChevronDown, ChevronUp } from 'lucide-react';
...
export const Customers: React.FC<CustomersProps> = ({ customers, setCustomers, settings, currentUser, t }) => {
  ...
  const [filter, setFilter]           = useState<'all'|'withPoints'|'highValue'>('all');
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'withPoints') list = list.filter(c => c.points > 0);
    if (filter === 'highValue')  list = list.filter(c => c.points >= (settings.minPointsForVoucher||100));
    return list;
  ...
  const formatDate = (d: string) => {
    ...
    return dt.toLocaleDateString(isRTL ? 'ar-MA' : 'en-GB');
  };
  ...
  const openEdit = (c: Customer) => {
    ...
    setForm({ name:c.name, phone:c.phone||'', email:c.email||'', address:c.address||'', notes:c.notes||'', totalSpent:c.totalSpent, points:c.points });
    setIsModalOpen(true);
    setExpandedId(null);
  };
    ...
    <div className="space-y-4 font-cairo" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex gap-1.5">
          {([['all','الكل',null],['withPoints','ذوو النقاط','orange'],['highValue','مؤهل للقسيمة','green']] as const).map(([id,label,color]) => (
            <button key={id} onClick={() => setFilter(id as any)}
              className={`px-3 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${filter===id ? (color==='orange'?'bg-orange-500 text-white':color==='green'?'bg-green-500 text-white':'bg-primary text-white') : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-2">
        {paged.length === 0 ? (
          ...
          const isEligible = c.points >= (settings.minPointsForVoucher||100);
          const isExpanded = expandedId === c.id;
          return (
            <div key={c.id} className={`bg-white dark:bg-gray-800 rounded-2xl border overflow-hidden transition-all ${isEligible?'border-green-400/40':'border-gray-100 dark:border-gray-700'}`}>
              ...
              <button onClick={() => setExpandedId(isExpanded ? null : c.id)}
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center font-black text-sm flex-shrink-0 ${isEligible?'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400':'bg-primary/10 text-primary'}`}>
                    {c.name.charAt(0)}
                  </div>
                  <div className="flex-1 min-w-0 text-start">
                    ...
                    <div className="text-xs text-gray-500 flex items-center gap-2">
                      ...
                      {c.points > 0 && <span className="text-orange-500 font-bold">⭐ {c.points.toFixed(1)}</span>}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {isEligible && <span className="hidden sm:inline text-[9px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-bold">🎫</span>}
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-3 text-center border border-gray-100 dark:border-gray-700">
                      <div className="text-lg font-black text-orange-500">{c.points.toFixed(1)}</div>
                      <div className="text-[10px] text-gray-500">{t.points||'نقطة'}</div>
                    </div>
                  </div>
                  {/* Contact info */}
            ...
            <div className="overflow-y-auto flex-1 p-4 space-y-3">
              ...
              {editingCustomer && (
                <div className="grid grid-cols-2 gap-3">
                  {[['totalSpent','إجمالي المشتريات'],['points','النقاط']].map(([k,l])=>(
                    <div key={k}>
                      <label className="text-xs font-bold text-gray-600 dark:text-gray-400 block mb-1">{l}</label>
                      <input type="number" value={(form as any)[k]||0}
