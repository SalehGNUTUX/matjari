...
export interface Product {
  ...
  priceHalfWholesale?: number; // سعر نصف الجملة
...
export type SaleTypeKey = 'retail' | 'wholesale' | 'halfWholesale' | 'installment';

export interface CartItem extends Product {
  quantity: number;
  saleType?: SaleTypeKey;   // نوع البيع لهذا العنصر بالسلة
  cartKey?: string;         // مفتاح فريد: id_saleType
}
...
export interface Sale {
  ...
  customerId?: string;
  customerName?: string;
  saleType?: SaleTypeKey;  // نوع البيع الرئيسي (أو mixed إن كانت مختلطة)
  pointsInfo?: {
    previous: number;
    earned: number;
    newTotal: number;
...
export interface AppSettings {
  ...
  enabledSaleTypes?: {         // أنواع البيع المتاحة في نقطة البيع
    retail: boolean;
    wholesale: boolean;
    halfWholesale: boolean;
    installment: boolean;
  };
