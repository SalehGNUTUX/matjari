...
import { ChevronDown, ChevronUp, LogOut, ShoppingBag, Lock, UserPlus, Trash2, ShieldAlert, Download, Github, Sun, Moon, Star, Users, RefreshCcw, X, UserCog, Edit2, RotateCcw, Settings2, Percent, Key, Globe, Shield, Gift, CreditCard, Languages } from 'lucide-react';
import { AppSettings, User as AuthUser, ReceiptSize, Language } from '../types';
import { openExternalLink } from '../src/capacitor-bridge';
...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const handleExportData = async () => {
    ...
    const filename = `matjari-backup-${dateStr}.json`;
  ...
  if (!isAdmin) {
    return (
      ...
      <p className="text-[10px] text-gray-400 font-bold mt-2">نظام متجري الذكي v2.9.5</p>
      </div>
      </div>
      ...
      <p className="text-[10px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-[0.2em]">{t.open_source_project || 'مشروع متجري مفتوح المصدر'}</p>
      <button
      onClick={() => openExternalLink("https://github.com/SalehGNUTUX/matjari")}
      className="flex items-center gap-3 text-white transition-all group-hover:text-primary"
      >
      <Github size={20} />
      <span className="text-lg font-black tracking-tight">SalehGNUTUX/matjari</span>
      </button>
      ...
      <Globe size={10} />
      <span>github.com/SalehGNUTUX/matjari</span>
      </div>
  ...
  return (
    ...
    placeholder="MAD أو درهم أو درهم مغربي"
    />
    ...
    <div className="flex items-center gap-3"><Star size={20} className="text-orange-400" /><span className="dark:text-white font-black text-sm">{t.enable_loyalty_points || 'تفعيل نقاط الوفاء'}</span></div>
    <button onClick={() => setSettings({...settings, pointsSystemEnabled: !settings.pointsSystemEnabled})} className={`w-14 h-7 rounded-full transition-all relative ${settings.pointsSystemEnabled ? 'bg-primary' : 'bg-gray-300 dark:bg-gray-700'}`}><div className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow-md transition-all ${settings.pointsSystemEnabled ? 'left-1' : 'left-8'}`}></div></button>
    </div>

    <div className="bg-gray-50 dark:bg-[#111827] p-5 rounded-3xl border border-gray-100 dark:border-white/5 relative">
    <label className="text-[10px] font-black text-gray-400 block mb-1 mr-2 uppercase absolute -top-2 right-4 bg-white dark:bg-[#1e293b] px-2">{t.loyalty_rate_percent || 'نسبة نقاط الزبائن (%)'}</label>
    ...
    onChange={e => {
      ...
      setSettings({...settings, loyaltyRate: value});
    }}
    ...
    <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-2">
    {settings.loyaltyRate === 1 ?
      t.default_loyalty_rate || 'افتراضي: 1% (1 نقطة لكل 100 درهم)' :
      `${settings.loyaltyRate}% (${settings.loyaltyRate} ${t.points_per_100 || 'نقطة لكل 100 درهم'})`
    }
    </p>
    </div>

    <div className="bg-gray-50 dark:bg-[#111827] p-5 rounded-3xl border border-gray-100 dark:border-white/5 relative">
    <label className="text-[10px] font-black text-gray-400 block mb-1 mr-2 uppercase absolute -top-2 right-4 bg-white dark:bg-[#1e293b] px-2">{t.min_points_for_voucher || 'الحد الأدنى للنقاط لقسيمة الشراء'}</label>
    <div className="flex items-center justify-center gap-2">
    <Gift size={20} className="text-green-500" />
    <input
    type="number"
    min="1"
    value={settings.minPointsForVoucher || 100}
    onChange={e => setSettings({...settings, minPointsForVoucher: Number(e.target.value)})}
    className="bg-transparent p-2 text-center dark:text-white font-black text-3xl outline-none w-24"
    />
    <span className="text-green-500 font-black text-xl">{t.point || 'نقطة'}</span>
    </div>
    <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-2">
    {t.min_points_note || 'عند وصول الزبون لهذا الحد، يصبح مؤهلاً لقسيمة شراء'}
    </p>
    </div>

    <div className="bg-gray-50 dark:bg-[#111827] p-5 rounded-3xl border border-gray-100 dark:border-white/5 relative group">
    <label className="text-[10px] font-black text-gray-400 block mb-1 mr-2 uppercase absolute -top-2 right-4 bg-white dark:bg-[#1e293b] px-2">{t.tax_percent || 'الضريبة (%)'}</label>
    <div className="flex items-center justify-center gap-2"><span className="text-primary font-black text-xl">%</span><input type="number" value={settings.taxRate * 100} onChange={e => setSettings({...settings, taxRate: Number(e.target.value)/100})} className="bg-transparent p-2 text-center dark:text-white font-black text-3xl outline-none w-24" /></div>
    </div>
    <div className="bg-gray-50 dark:bg-[#111827] p-5 rounded-3xl border border-gray-100 dark:border-white/5 relative">
    ...
    <h2 className="text-lg font-black text-gray-800 dark:text-white flex items-center gap-3 justify-end mb-6">
      {t.enabled_sale_types || 'أنواع البيع المتاحة'} <ShoppingBag size={20} className="text-primary" />
    </h2>
    <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 text-center">
      {t.sale_type_settings_note || 'يجب أن يبقى نوع واحد على الأقل مفعلاً في نقطة البيع'}
    </p>
    <div className="space-y-3">
      {([
        { key: 'retail',        icon: '🛍', label: t.sale_type_retail       || 'بالتجزئة',    desc: t.sale_type_retail_desc       || 'سعر التجزئة العادي' },
        { key: 'wholesale',     icon: '🏭', label: t.sale_type_wholesale    || 'بالجملة',     desc: t.sale_type_wholesale_desc    || 'سعر الجملة للكميات الكبيرة' },
        { key: 'halfWholesale', icon: '📦', label: t.sale_type_halfWholesale|| 'نصف جملة',    desc: t.sale_type_halfWholesale_desc|| 'سعر وسط بين التجزئة والجملة' },
        { key: 'installment',   icon: '📅', label: t.sale_type_installment  || 'بالتقسيط',    desc: t.sale_type_installment_desc  || 'سعر التقسيط للدفع المجزأ' },
      ] as const).map(st => {
        const enabled = settings.enabledSaleTypes?.[st.key] ?? true;
        const activeCount = ['retail','wholesale','halfWholesale','installment'].filter(k => settings.enabledSaleTypes?.[k as any] ?? true).length;
        const canDisable = activeCount > 1 || !enabled;
        return (
          <div key={st.key} className="bg-gray-50 dark:bg-[#111827] p-4 rounded-2xl flex items-start justify-between gap-3 border border-gray-100 dark:border-white/5">
            ...
            <button
              onClick={() => {
                ...
                setSettings(s => ({
                  ...s,
                  enabledSaleTypes: {
                    retail: s.enabledSaleTypes?.retail ?? true,
                    wholesale: s.enabledSaleTypes?.wholesale ?? true,
                    halfWholesale: s.enabledSaleTypes?.halfWholesale ?? true,
                    installment: s.enabledSaleTypes?.installment ?? true,
                    [st.key]: !enabled,
                  }
                }));
              }}
              disabled={!canDisable}
              className={`w-14 h-7 rounded-full transition-all relative flex-shrink-0 disabled:opacity-40 disabled:cursor-not-allowed ${enabled ? 'bg-primary' : 'bg-gray-300 dark:bg-gray-700'}`}
              title={!canDisable ? (t.sale_type_settings_note || 'يجب أن يبقى نوع واحد على الأقل') : ''}>
              <div className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow-md transition-all ${enabled ? 'left-1' : 'left-8'}`}></div>
            </button>
          </div>
        );
      })}
    ...
    <div className="mt-4 bg-gray-50 dark:bg-[#111827] p-4 rounded-2xl flex items-start justify-between gap-3 border border-gray-100 dark:border-white/5">
      <div className="flex-1 min-w-0">
        <div className="font-black text-sm text-gray-800 dark:text-white flex items-center gap-2">
    ...
    <div className="mt-3 bg-yellow-50 dark:bg-yellow-900/10 p-4 rounded-2xl flex items-start justify-between gap-3 border border-yellow-200 dark:border-yellow-500/20">
      <div className="flex-1 min-w-0">
        <div className="font-black text-sm text-gray-800 dark:text-white flex items-center gap-2">
