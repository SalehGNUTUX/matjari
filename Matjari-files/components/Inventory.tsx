...
import {
  ...
  RefreshCcw, Camera, Truck, ChevronDown
} from 'lucide-react';
  ...
  return (
    ...
    {isModalOpen && (
      ...
      <div className="col-span-2 space-y-3 pt-2">
        ...
        {/* صف 2: جملة + نصف جملة + تقسيط */}
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-orange-500/5 p-3 rounded-2xl border border-orange-500/10 text-center">
            <label className="text-[9px] font-black text-orange-500 block mb-1">🏭 جملة</label>
            <input type="number" value={formData.priceWholesale || ''}
              onChange={e => setFormData({...formData, priceWholesale: Number(e.target.value)})}
              onFocus={e => e.target.select()}
              placeholder="0"
              className="w-full bg-transparent dark:text-white text-base font-black text-center outline-none" />
          </div>
          <div className="bg-yellow-500/5 p-3 rounded-2xl border border-yellow-500/10 text-center">
            <label className="text-[9px] font-black text-yellow-600 block mb-1">½ نصف</label>
            <input type="number" value={formData.priceHalfWholesale || ''}
              onChange={e => setFormData({...formData, priceHalfWholesale: Number(e.target.value)})}
              onFocus={e => e.target.select()}
              placeholder="0"
              className="w-full bg-transparent dark:text-white text-base font-black text-center outline-none" />
          </div>
          <div className="bg-pink-500/5 p-3 rounded-2xl border border-pink-500/10 text-center">
            <label className="text-[9px] font-black text-pink-500 block mb-1">📅 تقسيط</label>
