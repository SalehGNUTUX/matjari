...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const dailyData = Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        day: d.toLocaleDateString('ar-MA', { weekday: 'short' }),
  ...
  const handleExport = () => {
    const data = { timestamp: new Date().toISOString(), sales, products, customers, suppliers, users, settings };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
    a.download = `matjari-backup-${Date.now()}.json`; a.click();
  };
    ...
    <div className="space-y-5 font-cairo" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">{new Date().toLocaleDateString('ar-MA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
              ...
              <div key={u.id} className="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {u.lastLogin && !u.isOnline && <div className="text-xs text-gray-400 mt-0.5">آخر دخول: {new Date(u.lastLogin).toLocaleString('ar-MA')}</div>}
                  </div>
                </div>
          ...
          {debts.filter(d => d.status !== 'paid').slice(0, 5).map(d => (
            <div key={d.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-gray-500">{new Date(d.createdAt).toLocaleDateString('ar-MA')}</div>
              </div>
