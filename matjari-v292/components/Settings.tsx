...
export const Settings: React.FC<SettingsProps> = ({
  settings, setSettings, users, setUsers, currentUser, onUpdateCurrentUser, onLogout, onResetSystem, products, sales, customers, suppliers, t = {}
}) => {
  ...
  const [securityPending, setSecurityPending] = useState<keyof AppSettings['security'] | 'staff-management' | 'reset-system' | null>(null);
  ...
  const handleExportData = () => {
    const data = { products, customers, suppliers, sales, settings, users };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `matjari-backup-${new Date().toLocaleDateString()}.json`;
    link.click();
  };
  ...
  if (!isAdmin) {
    return (
      ...
      <p className="text-[10px] text-gray-400 font-bold mt-2">نظام متجري الذكي v2.9</p>
      </div>
      </div>
