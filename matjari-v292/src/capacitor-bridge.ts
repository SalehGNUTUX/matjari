...
export const printOrShare = async (
  ...
  if (isNativeAndroid()) {
    try {
      ...
      // Fallback: share HTML if canvas failed
      ...
      await Share.share({ title: `فاتورة متجري #${saleId}`, url: uri2, dialogTitle: 'مشاركة الفاتورة' });
      return { success: true };
...
export const saveImage = async (
  ...
  if (isNativeAndroid()) {
    try {
      ...
      // Share sheet lets user pick: save to gallery, WhatsApp, Drive…
      await Share.share({
        title: filename,
        url: uri,
        dialogTitle: 'حفظ أو مشاركة الصورة',
      });
      return { success: true };
    } catch { /* fallback */ }
  }
